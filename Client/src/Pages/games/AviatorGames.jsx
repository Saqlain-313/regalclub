...
const AviatorGames = () => {
  ...
  const aviatorGame = {
    ...
    provider: "SPB",
    icon:
      "http://files.worldcasinoonline.com/Document/Game/Aviator_1697879631441.088.png",
    rating: 4.9,
    players: "5.6K",
    volatility: "High",
    min_bet: 10,
  ...
  // --------------------------------------------------
  // RESET ONLY ON COMPONENT UNMOUNT
  // --------------------------------------------------
  ...
  // --------------------------------------------------
  // OPEN GAME MODAL WHEN URL IS AVAILABLE
  // --------------------------------------------------
  ...
  const handlePlay = async () => {
    ...
    try {
      ...
      console.error("Failed to launch Aviator:", error);
  ...
  const handleReconnect = async () => {
    try {
      ...
      console.error("Failed to reconnect Aviator:", error);
  ...
  const closeGameModal = () => {
    ...
    // Clear game URL/state
    dispatch(clearGameUrl());

    /*
      IMPORTANT:
      GamePlayModal ka Back button yahi function call karta hai.

      Pehle sirf modal close ho raha tha.
      Ab browser history me previous route par jayega.
    */
    navigate(-1);
  };
